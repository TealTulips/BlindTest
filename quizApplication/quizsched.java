...
// Print the quiz retake choices (maybe should also change the method name?)
	static void printQuizScheduleForm(quizzes quizList, retakes retakesList, courseBean course) {
		...
		// Checks for both today and endDay between startSkip and endSkip & add 7 to
		// endDay
			...
			if (!(retakeDay.isBefore(today)) && !(retakeDay.isAfter(endDay))) {
				...
				if (!((retakeDay.isAfter(startSkip) && retakeDay.isBefore(endSkip)) || retakeDay.equals(startSkip)
					...
					for (quizBean q : quizList) {
						...
						// if skip week is true increase retake duration by 7
						...
						// check quiz time and day for retake slots
