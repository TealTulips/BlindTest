...
/**
 *
 * Wiring the pieces together: quizschedule.java -- Servlet entry point for
 * students to schedule quizzes quizReader.java -- reads XML file and stores in
 * quizzes. Used by quizschedule.java quizzes.java -- A list of quizzes from the
 * XML file Used by quizschedule.java quizBean.java -- A simple quiz bean Used
 * by quizzes.java and readQuizzesXML.java retakesReader.java -- reads XML file
 * and stores in retakes. Used by quizschedule.java retakes.java -- A list of
 * retakes from the XML file Used by quizschedule.java retakeBean.java -- A
 * simple retake bean Used by retakes.java and readRetakesXML.java apptBean.java
 * -- A bean to hold appointments
 *
 * quizzes.xml -- Data file of when quizzes were given retakes.xml -- Data file
 * of when retakes are given course.xml -- Data file with information about the
 * course
 */
...
/*
 * FAULTY * grep -v FAULTY quizsched-FAULTS.java to remove the comments and
 * leave the faults.
 */
/* FAULTY * Known faults: */
/*
 * FAULTY * 1. Doesn't check for today in skip week, only endDate checked. Or if
 * skip week between today and endDay not checked. If just today is in
 * skipweek,should add 7 days.
 */
/*
 * FAULTY * 2. If skip week started before last available day of quiz
 * i.e.startSkip or endSkip between quizDay and lastAvailable day then extra
 * days not added to the number of retakes.
 */
/*
 * FAULTY * 3. Retake available on the 14th day from the day of the quiz after
 * the class too. Should be available only before the quiz time.
 */
/* FAULTY * 4. Skip week msg printing position */
/* FAULTY * 5. Shows quiz and retake on skip week too */

public class quizsched {
	// Used to pass retakeID,quizID pairs from printQuizScheduleForm() to
	// readInputSave()
...
// Prints the form to schedule a retake
	public static void main(String[] argv) /* CLI */
	{
		...
		try {
			...
			System.out.println("Can't find the data files for course ID " + courseID
					+ ". You can try again with a different course ID." + e);
			return;
		}
		...
		try { // Read the files and print the form
			...
			System.out.println("Can't read the data files for course ID " + courseID
					+ ". You can try again with a different courseID." + e);
			return;
		}
...
//=========================================
	public Properties getProperty() {
		return quizsched.retakeQuizIDProps;
	}
...
// Called from main to read student's choice and save to file
	private static void readInputSave(Scanner sc, String courseID) /* CLI */
	{
		...
		String retakeQuizID = retakeQuizIDProps.getProperty(retake); /* CLI */ // returns the ith retake of a quiz
...
// Print the quiz retake choices (maybe should also change the method name?)
	public static void printQuizScheduleForm(quizzes quizList, retakes retakesList, courseBean course) {
		...
		/*
		 * Faulty * fault 1, should check if endDay and today is between startSkip and
		 * endSkip
		 */
		...
		for (retakeBean r : retakesList) {
			...
			/*
			 * FAULTY * fault 5, only checks for two week duration, shows retake during skip
			 * week too
			 */
			...
			if (!(retakeDay.isBefore(today)) && !(retakeDay.isAfter(endDay))) {
				...
				/*
				 * FAULTY * fault 4, prints skip message for retake after 2 week period instead
				 * of the first retake after skip week.
				 */
				...
				System.out.println("RETAKE: " + retakeDay.getDayOfWeek() + ", " + retakeDay.getMonth() + " "
				...
				for (quizBean q : quizList) {
					...
					/* FAULTY * fault 2, doesn't increase lastAvailableDay by 7 */
...
// Read the course file
	...
	public courseBean getCourseFile(String courseId) throws Exception {
		buildFileNames(courseId);
		return readCourseFile(courseId);
	}
...
// Read the quizzes file
	...
	public quizzes getQuizzes(String courseId) throws Exception {
		return readQuizzes(courseId);
	}
...
// Read the retakes file
	...
	public retakes getRetakes(String courseId) throws Exception {
		return readRetakes(courseId);
	}
